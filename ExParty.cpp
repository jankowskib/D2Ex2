...
//BOOL __fastcall ExParty::GetKillCount(BYTE* aPacket) //0x65 HANDLE SRV->CLT
//{
...
//{
//	if (i->Name == szName) { i->Kills=aCount;	break;	}
//}
//ExParty::Update();
//return 0;
//}
...
//{
//	i->Location->SetText(GetLvlNameEx(pData->Location,145));
//	break;
//}
//}
//return 0;
...
for(list<AccountBase>::iterator i = AccBase.begin() ; i != AccBase.end() ; i++)
{
if (i->Name == szName && !i->Account.empty()) return i->Account;
}
...
for(list<AccountBase>::iterator i = AccBase.begin() ; i != AccBase.end() ; i++)
{
	if (i->Name == szName && !i->Account.empty()) { GetPrivateProfileString("D2ExClans",i->Account.c_str(),"N/A",szReturn,5,ClansIni.c_str()); break; }
}
return szReturn;
}
...
//================================================================
	...
	if(!ptRoster) { ExParty::Refresh(); return;}
	...
	if(ptRoster->dwUnitId!=D2Funcs::D2CLIENT_GetPlayer()->dwUnitId)
		{
			...
			Tbl.Squelch = new ExButton(Tbl.Frame->GetX()+460,yPos,0,8,L"",CellFiles::PARTY,&ExParty::Squelch,0);
			PartyScreen->AddChild(Tbl.Squelch);
		} 
		/*else
		Tbl.Location->SetText(GetLvlNameEx(ExParty::GetPlayerArea(),145));*/
...
Clan = new ExTextBox(PartyScreen->GetX()+201,105,0,0,LocId == 10 ? L"Klan" : L"Clan",0,0);
Acc = new ExTextBox(PartyScreen->GetX()+237,105,0,0,wAccStr,0,0);
//Location = new ExTextBox(PartyScreen->GetX()+346,105,0,0,LocId == 10 ? L"Lokacja" : L"Location",0,0);
Kills = new ExImage(PartyScreen->GetX()+346,105,5,1,CellFiles::MONINDICATOR); Kills->Hoover = LocId == 10 ? L"Zabójstwa" : L"Kills";
