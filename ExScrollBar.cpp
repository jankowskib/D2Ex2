...
#include "Misc.h"
	...
	void Increase(ExControl*tt){ 
		...
		*Parent->sValue= (*Parent->sValue) + 1; 
		Misc::Debug("sVal %d, sMax %d, sMin %d",*Parent->sValue,Parent->sMax,Parent->sMin);
		Parent->Update();
		}
	}
...
ExScrollBar::ExScrollBar(int X, int Y, int Min, int Max, int Height,int * Var, void (*efnOnChange)(ExControl*)) :
aCellFile(CellFiles::SCROLLBAR), ExControl(X,Y,13,Height,0)
{
if(!aCellFile.Get()) 
{
...
Misc::Debug("c = %d, ts =%f, o = %d, %d", c, ts, o, GetHeight());
Slider->SetY(cY+12+o);
...
/*PGUP*/	case VK_PRIOR:{if(*sValue>=sMin+5) { *sValue = *sValue - 5; Update();} else if(*sValue>sMin) {*sValue = sMin; Update(); }  return true;}
/*PGDN*/	case VK_NEXT: {if(*sValue<=sMax-5) { *sValue = *sValue + 5; Update();}  else if(*sValue<sMax) {*sValue = sMax; Update(); }  return true;}
