...
ExImage::ExImage(int X, int Y, int Transp, unsigned int Frame, string szFile) : aCellFile(szFile), ExControl(X,Y,-1,-1,0)
{
	if(!aCellFile.Get() || !aCellFile.GetCF()) {
...
{
//#ifdef _DEBUG
//Misc::Log("-->Zniszczylem obiekt ExImage (X=%d, Y=%d)!",cX,cY);
//#endif
