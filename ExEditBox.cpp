...
ExEditBox::ExEditBox(int X, int Y, int TextXOffset, int TextYOffset, int TextLen, int TextFont, wstring szLabel, string szFile) : aCellFile(szFile), ExControl(X,Y,-1,-1,0)
{
...
if(*D2Vars::D2CLIENT_MouseX>=cX && *D2Vars::D2CLIENT_MouseX<=cX+cWidth && *D2Vars::D2CLIENT_MouseY<=cY && *D2Vars::D2CLIENT_MouseY>=cY-cHeight) 
{
if(cState==VISIBLE) { D2Funcs::D2CLIENT_PlaySound(ExSounds::STAND_CLICK); isFocused = true;
}
bBeingPressed=false;
return true;
}
