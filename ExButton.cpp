...
ExButton::ExButton(int bX, int bY, int TextColor, unsigned int Frame, wstring szLabel, string szFile, void (*tevent_onClick)(ExControl*), bool LongButton) : aCellFile(szFile), ExControl(bX,bY,-1,-1,tevent_onClick)
{
ASSERT(aCellFile.Get())
aTextBox = 0;
Color = 0;
...
if(bBeingPressed){
	...
	if(aTextBox) {_FixText(); aTextBox->SetY(aTextBox->GetY()+1); aTextBox->SetX(aTextBox->GetX()+2);}
	}
else 
	{ 
	aCellFile.Get()->nCellNo=aFrame; 
	if(aTextBox) _FixText();
	}
